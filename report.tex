...
In our experiment we test for each algorithm (BubbleSortPassPerItem, BubbleSortUntilNoChange, BubbleSortWhileNeeded) each combination of the element type of the array with different characteristic of the array.
This means that each algorithm is tested for:\\
(int, [random, sorted, partially sorted, reverse sorted, with duplicates, with no duplicates, equal elements]),\\ 
(Integer, [random, sorted, partially sorted, reverse sorted, with duplicates, with no duplicates, equal elements]), \\
(String, [random, sorted, partially sorted, reverse sorted, with duplicates, with no duplicates, equal elements]), \\
(double, [random, sorted, partially sorted, reverse sorted, with duplicates, with no duplicates, equal elements]), \\
(char, [random, sorted, partially sorted, reverse sorted, with duplicates, with no duplicates, equal elements]), \\
(byte, [random, sorted, partially sorted, reverse sorted, with duplicates, with no duplicates, equal elements]).\\
Furthermore it has repeated measures because each of the previous array-type combination is tested 150 times for which the first 25 iterations are not counted as actual values but as warm-up values.
\\\\
